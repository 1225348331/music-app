<script setup>
import { NScrollbar, NDropdown, NButton, NIcon, NText, NFlex } from "naive-ui";
import { ref, computed, h } from "vue";
import { useRouter } from "vue-router";
import Menu from "@/components/Global/Menu.vue";
import SvgIcon from "@/components/Global/SvgIcon.vue";
import SearchInp from "@/components/Search/SearchInp.vue";
...
const mainMenuOptions = computed(() => [
  {
    ...
    render: () => {
      return h(
        NScrollbar,
        { style: { maxHeight: "calc(100vh - 200px)", minWidth: "280px" } },
        () => h(Menu)
      );
    },
  },
  ...
  <nav :class="{ 'main-nav': true }">
    <div class="left">
      <div :class="['logo']" @click="router.push('/')">
        <n-icon class="logo-img" size="30"> </n-icon>
        <Transition name="fade" mode="out-in">
          <n-text class="site-name"> AugPlayer </n-text>
        </Transition>
      </div>
